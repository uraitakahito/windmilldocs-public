---
title: 'PHP クイックスタート'
description: 'Windmill で PHP のスクリプトを書くには。Composer による依存関係の管理とあわせて、作り、テストし、配備する。'
...
- [コード](#コード): PHP のスクリプトでは、少なくとも `main` 関数が要ります。
- [設定](#設定): パス・要約・説明・入力の [JSON Schema](../../../core_concepts/13_json_schema_and_parsing/index.mdx)（署名から推論されます）といった、スクリプトの設定とメタデータ。

コードリポジトリに保存すると、この 2 つは `<path>.php` と `<path>.script.yaml` に分かれて置かれます。

[依存関係](../../../advanced/6_imports/index.mdx)は Windmill が自動で扱います。PHP のスクリプトでライブラリを import すると、保存時に Windmill がその import を読み取り、依存関係の一覧を自動で作ります。そのうえで依存解決の job を起こし、Composer のパッケージを lockfile に結び付けます。これにより、同じ版のスクリプトは常に同じ版の依存関係で実行されます。

Windmill で PHP で組んだスクリプトの簡単な例を挙げます。
...
この例では `php` を選んだので、Windmill が PHP の雛形を用意してくれています。見てみましょう。
...
パッケージは composer で入れられます。`require` の行のコメントを外して、必要なパッケージを書き足すだけです。あとは Windmill が入れてくれます。
...
- `<?php` の開始タグの下に `// volume:` の註記を書いて[ボリューム](../../../core_concepts/67_volumes/index.mdx)をマウントすると、実行をまたいでファイルを残せます（[Enterprise Edition](/pricing)）。
